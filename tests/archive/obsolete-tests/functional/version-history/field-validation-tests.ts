...
export class FieldValidationTests {
    ...
    const tests = [
      {
        ...
        test: async () => {
          ...
          try {
            const versionHistoryRes = await this.makeRequestWithRetry(
  ...
  private validatePageUsage(page: string, fieldName: string, value: any) {
    ...
    const validPages = ['VersionHistoryPage', 'ProposalList', 'CustomerList', 'ProductList'];
    ...
    switch (page) {
      case 'VersionHistoryPage':
        // Version history specific validations
        if (fieldName === 'version' && value !== undefined) {
          if (value < 1) {
            return { valid: false, error: 'Version must be >= 1' };
          }
        }
        if (fieldName === 'changeType' && value !== undefined) {
          const validTypes = [
            'create',
            'update',
            'delete',
          ...
          if (!validTypes.includes(value)) {
            return { valid: false, error: `Invalid change type: ${value}` };
          }
        }
        break;
  ...
  private getVersionHistoryFields() {
    return {
      id: {
        dbType: 'String',
        zodType: 'string',
        ...
        relationship: null,
        description: 'Primary key for version history entry',
      },
      proposalId: {
        dbType: 'String',
        zodType: 'string',
      ...
      metadata: {
        ...
        page: 'VersionHistoryPage',
        relationship: null,
        description: 'Additional metadata for version entry',
      },
      createdAt: {
        dbType: 'DateTime',
        zodType: 'string',
