...
export class ApiClient {
  ...
  // Generate unique proposal ID to avoid database conflicts
  private generateUniqueProposalId(prefix: string = 'PROP'): string {
    const uniqueId = this.generateUniqueId('prop');
    return `${prefix}-${uniqueId}`.substring(0, 50); // Proposal ID field limit
  }
