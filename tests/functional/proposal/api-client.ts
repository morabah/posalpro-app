...
/**
 * PosalPro MVP2 - Enhanced API Client for Proposal Functional Testing
...
export class ApiClient {
  ...
  // Generate unique proposal title
  private generateUniqueTitle(prefix: string = 'Proposal'): string {
    const uniqueId = this.generateUniqueId('proposal');
    ...
    try {
      ...
      if (sessionRes.status === 200 && sessionData?.user) {
        logInfo('Functional test login successful', {
          component: 'ProposalFunctionalTester',
          operation: 'login',
          email,
  ...
  // Proposal-specific helper methods
  async createTestProposal(proposalData?: any) {
    const uniqueTitle = this.generateUniqueTitle('Test Proposal');

    const testProposal = proposalData || {
      title: uniqueTitle,
      description: 'Test proposal for functional testing',
    ...
    const response = await this.request('POST', '/api/proposals', testProposal);
    ...
    throw new Error(`Failed to create test proposal: ${response.status}`);
  }

  async cleanupTestProposal(proposalId: string) {
    try {
      await this.request('DELETE', `/api/proposals/${proposalId}`);
    } catch (error) {
      // Ignore cleanup errors in tests
      logError('Proposal cleanup failed', {
        component: 'ProposalFunctionalTester',
        operation: 'cleanup',
        proposalId,
        error: error instanceof Error ? error.message : 'Unknown error',
