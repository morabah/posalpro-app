...
export default function RelationshipsWorkspacePage() {
    ...
    return (
      <div className="flex items-center justify-center min-h-screen">
        <div className="text-center">
          ...
          <p className="text-sm text-gray-600">Loading product relationships...</p>
        </div>
      </div>
    );
  }
    ...
    (async () => {
      try {
        const res = await bridge.fetchProducts({
          limit: 100,
          fields: 'id,sku,name,status',
        });
        const list: ProductSummary[] = (res.success && res.data ? res.data : []).map(p => {
          // Normalize status to match ProductSummary interface
          let status: 'Active' | 'Draft' | 'Deprecated' = 'Active';
          if (p.status) {
            const statusStr = String(p.status).toLowerCase();
            if (statusStr === 'draft') status = 'Draft';
            else if (statusStr === 'deprecated') status = 'Deprecated';
            else if (statusStr === 'inactive' || statusStr === 'discontinued')
              status = 'Deprecated';
          }
  ...
  const loadRules = useCallback(
    async (productId: string) => {
      setLoading(true);
      try {
        // Note: Rules loading would need a specific bridge method
        // For now, using empty array as placeholder
        const rules: ProductRelationshipRuleDTO[] = [];
        setRules(rules);
        analytics('relationships_rules_loaded', { productId, count: rules.length }, 'medium');
      } catch (error) {
  ...
  }, [selectedProductId]); // Remove loadRules dependency to prevent infinite loops
