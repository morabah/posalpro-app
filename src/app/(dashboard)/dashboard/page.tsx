...
// 🚀 PERFORMANCE OPTIMIZATION: Unified dashboard data loading
import { useUnifiedDashboardData } from '@/features/dashboard';
...
const UnifiedProposalList = dynamic(() => import('@/components/proposals/UnifiedProposalList'), {
...
const UnifiedDashboardStats = dynamic(
  () =>
    import('@/components/dashboard/UnifiedDashboardStats').then(mod => ({
      default: mod.UnifiedDashboardStats,
    })),
  {
    loading: () => <div className="animate-pulse bg-gray-200 rounded-lg h-64" />,
  }
 ...
 * 🚀 PERFORMANCE OPTIMIZATION: Unified data loading with parallel API calls
 */
export default function DashboardPage() {
  // 🚀 PERFORMANCE OPTIMIZATION: Load all dashboard data in parallel
  const { data: unifiedData, isLoading, error } = useUnifiedDashboardData('3M');

  // Extract data from unified response
  const executiveData = unifiedData?.executive || null;
  const enhancedData = unifiedData?.enhanced || null;
  const analyticsData = unifiedData?.analytics || null;
  return (
