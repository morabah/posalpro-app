...
/*
 ...
 🚀 ALWAYS USE THIS CLI FOR:
   ...
   - RBAC permission testing with comprehensive endpoint, matrix, policy, multi-tenant, and performance testing
   - Multi-session authentication management

 📋 AVAILABLE COMMANDS:
   - Authentication: login, login-as, use-session, whoami, logout
   - Database: detect-db, db, health, health:db, export, import, versions
   - Monitoring: monitor, health, health:api, health:redis, redis
   ...
   - Entity Operations: customers create/update/delete, products create/update/delete, proposals create/update/get/patch-products/patch-manual-total/cache/backfill/add-product/update-product/remove-product/snapshot
  - Data Consistency: consistency check [entity] [id] # Check list vs detail view consistency
  - Data Consistency: consistency check-all # Check all entities for data inconsistencies
  - Data Consistency: consistency validate [entity] [id] # Validate specific entity data
   - Version Management: versions list, versions for <id>, versions diff <id> <version>, versions assert
   - RBAC Testing: rbac try <method> <endpoint> [role], rbac run-set, rbac test-roles, rbac endpoints, rbac matrix, rbac policies, rbac multi-tenant, rbac performance
   - Entitlements Testing: entitlements test, entitlements check
   - Troubleshooting: troubleshoot auth, troubleshoot dashboard
   - Redis: redis status, redis health, redis connect, redis get/set/del/keys/clear/flush
   - Tests: tests tenant
 ...
 Usage Examples:
  ...
  npm run app:cli -- --command "schema test generate customers.CustomerCreateSchema" # generate test data from schema
   ...
   npm run app:cli -- --command "consistency check proposals cmfsfm71q001wrx1hq8msnmgr" # check specific entity consistency
   npm run app:cli -- --command "consistency check-all" # check all entities for inconsistencies
   npm run app:cli -- --command "consistency validate proposals cmfsfm71q001wrx1hq8msnmgr" # validate specific entity data
  ...
  switch (cmd) {
    ...
    case 'rbac': {
      ...
      if (sub === 'try') {
        ...
        await handleRBACEndpointsTest(api);
      } else if (sub === 'matrix') {
        await handleRBACPermissionMatrixTest(api);
      } else if (sub === 'policies') {
        await handleRBACPolicyValidation(api);
      } else if (sub === 'multi-tenant') {
        await handleRBACMultiTenantTest(api);
      } else if (sub === 'performance') {
        await handleRBACPerformanceTest(api);
      } else {
        console.log('Usage:');
        console.log('  rbac try <method> <path> [json]            # Test single API call');
        console.log('  rbac run-set [file]                        # Run test suite from file');
        console.log('  rbac test-roles [file]                     # Test multiple user roles');
        console.log(
          '  rbac endpoints                             # Test all RBAC-protected endpoints'
        );
        console.log('  rbac matrix                                # Test permission matrix');
        console.log('  rbac policies                              # Validate RBAC policies');
        console.log('  rbac multi-tenant                          # Test multi-tenant RBAC');
        console.log('  rbac performance                           # RBAC performance testing');
      }
      break;
    }
    case 'versions': {
    ...
    case 'consistency': {
      ...
      if (sub === 'check') {
        if (!entity) {
          ...
          console.log('Available entities: proposals, customers, products');
          console.log('Example: consistency check proposals cmfsfm71q001wrx1hq8msnmgr');
        } else {
          await handleDataConsistencyCheck(api, entity, id);
        }
      } else if (sub === 'check-all') {
        await handleDataConsistencyCheckAll(api);
      } else if (sub === 'validate') {
        if (!entity || !id) {
          console.log('Usage: consistency validate <entity> <id>');
          console.log('Example: consistency validate proposals cmfsfm71q001wrx1hq8msnmgr');
        } else {
          await handleDataConsistencyValidate(api, entity, id);
        }
      } else {
        console.log(
          'Usage: consistency check <entity> [id] | consistency check-all | consistency validate <entity> <id>'
        );
      }
      break;
    }
...
// ============================================================================
// RBAC TESTING FUNCTIONS
// ============================================================================

async function handleRBACEndpointsTest(api: ApiClient) {
  console.log('🛡️ PosalPro MVP2 - RBAC Endpoints Test\n');

  const rbacEndpoints = [
    // Admin endpoints
    { method: 'GET', path: '/api/admin/roles', roles: ['System Administrator', 'Administrator'] },
    { method: 'POST', path: '/api/admin/roles', roles: ['System Administrator'] },
    { method: 'PUT', path: '/api/admin/roles', roles: ['System Administrator'] },
    { method: 'DELETE', path: '/api/admin/roles', roles: ['System Administrator'] },
    ...
    { method: 'POST', path: '/api/admin/permissions', roles: ['System Administrator'] },
    { method: 'PUT', path: '/api/admin/permissions', roles: ['System Administrator'] },
    { method: 'DELETE', path: '/api/admin/permissions', roles: ['System Administrator'] },
    ...
    { method: 'POST', path: '/api/admin/users/roles', roles: ['System Administrator'] },
    { method: 'PUT', path: '/api/admin/users/roles', roles: ['System Administrator'] },
    { method: 'DELETE', path: '/api/admin/users/roles', roles: ['System Administrator'] },

    { method: 'GET', path: '/api/admin/metrics', roles: ['System Administrator', 'Administrator'] },
    {
      method: 'GET',
      path: '/api/admin/users/stats',
      roles: ['System Administrator', 'Administrator'],
    },
    { method: 'GET', path: '/api/admin/system', roles: ['System Administrator'] },

    // Tenant entitlements
    {
      method: 'GET',
      path: '/api/tenant/entitlements',
      roles: ['System Administrator', 'Administrator', 'Proposal Specialist'],
    },
    ...
    // Protected API endpoints
    {
      method: 'GET',
      path: '/api/proposals',
      roles: ['System Administrator', 'Administrator', 'Proposal Specialist', 'Proposal Manager'],
    },
    {
      method: 'POST',
      path: '/api/proposals',
...

    {
      method: 'GET',
      path: '/api/customers',
      roles: ['System Administrator', 'Administrator', 'Proposal Specialist', 'Customer Manager'],
    },
    {
      method: 'POST',
      path: '/api/customers',
...

    {
      method: 'GET',
      path: '/api/products',
      roles: ['System Administrator', 'Administrator', 'Proposal Specialist', 'Product Manager'],
    },
    {
      method: 'POST',
      path: '/api/products',
      roles: ['System Administrator', 'Administrator', 'Product Manager'],
    },
  ];

  let total = rbacEndpoints.length;
  let passed = 0;
  let failed = 0;

  console.log(`Testing ${total} RBAC-protected endpoints...\n`);

  for (const endpoint of rbacEndpoints) {
    try {
      const res = await api.request(endpoint.method as HttpMethod, endpoint.path);
      const allowed = res.status < 400;
      const status = res.status;

      console.log(
        `[${endpoint.method}] ${endpoint.path} → ${allowed ? '✅ Allowed' : '❌ Denied'} (${status}) [${endpoint.roles.join(', ')}]`
      );
      ...
      console.log(
        `[${endpoint.method}] ${endpoint.path} → ❌ Error: ${error instanceof Error ? error.message : 'Unknown error'}`
      );
      failed++;
    }
  }

  console.log(`\n📊 RBAC Endpoints Test Results:`);
...
async function handleRBACPermissionMatrixTest(api: ApiClient) {
  console.log('🔐 PosalPro MVP2 - RBAC Permission Matrix Test\n');

  const testUsers = [
    {
      role: 'System Administrator',
      expected: { admin: true, create: true, read: true, update: true, delete: true },
    },
    {
      role: 'Administrator',
      expected: { admin: true, create: true, read: true, update: true, delete: true },
    },
    {
      ...
      expected: { admin: false, create: true, read: true, update: true, delete: false },
    },
    {
    ...
    {
      role: 'Viewer',
      expected: { admin: false, create: false, read: true, update: false, delete: false },
    },
  ];

  const testEndpoints = [
    { path: '/api/admin/roles', method: 'GET', permission: 'admin' },
    { path: '/api/proposals', method: 'POST', permission: 'create' },
    { path: '/api/proposals', method: 'GET', permission: 'read' },
    { path: '/api/proposals', method: 'PUT', permission: 'update' },
    { path: '/api/proposals', method: 'DELETE', permission: 'delete' },
  ];
  ...
  for (const user of testUsers) {
    ...
    for (const endpoint of testEndpoints) {
      try {
        const res = await api.request(endpoint.method as HttpMethod, endpoint.path);
        const allowed = res.status < 400;
        const expected = user.expected[endpoint.permission as keyof typeof user.expected];

        const result = allowed === expected ? '✅' : '❌';
        console.log(
          `[${user.role}] ${endpoint.method} ${endpoint.path} → ${allowed ? 'Allowed' : 'Denied'} (${res.status}) | Expected: ${expected ? 'Allowed' : 'Denied'} ${result}`
        );
        ...
        const expected = user.expected[endpoint.permission as keyof typeof user.expected];
        const result = false === expected ? '✅' : '❌';
        console.log(
          `[${user.role}] ${endpoint.method} ${endpoint.path} → ❌ Error | Expected: ${expected ? 'Allowed' : 'Denied'} ${result}`
        );
        if (false === expected) {
          passedTests++;
        }
  ...
  console.log(`\n📊 RBAC Permission Matrix Test Results:`);
  console.log(`   Total tests: ${totalTests}`);
  console.log(`   Passed: ${passedTests} ✅`);
  console.log(`   Failed: ${totalTests - passedTests} ❌`);
  console.log(`   Success rate: ${((passedTests / totalTests) * 100).toFixed(1)}%`);

  if (totalTests - passedTests > 0) process.exitCode = 1;
}

async function handleRBACPolicyValidation(api: ApiClient) {
  console.log('🔍 PosalPro MVP2 - RBAC Policy Validation\n');

  const policyTests = [
    // Admin access policies
    {
      description: 'Admin should access all admin endpoints',
      method: 'GET',
      path: '/api/admin/roles',
      expectRole: 'System Administrator',
    },
    {
      description: 'Admin should access system metrics',
      method: 'GET',
    ...
    {
      description: 'User role management should be admin-only',
      method: 'POST',
      path: '/api/admin/users/roles',
      expectRole: 'System Administrator',
    },
    {
      description: 'Permission management should be admin-only',
      method: 'POST',
      path: '/api/admin/permissions',
      expectRole: 'System Administrator',
    },
    ...
    {
      description: 'Proposal creation should be role-based',
      method: 'POST',
      path: '/api/proposals',
      expectRole: 'Proposal Specialist',
    },
    {
      description: 'Customer management should be role-based',
      method: 'POST',
      path: '/api/customers',
      expectRole: 'Customer Manager',
    },

    // Tenant isolation policies
    {
      description: 'Tenant entitlements should be accessible',
      method: 'GET',
      path: '/api/tenant/entitlements',
      expectRole: 'Proposal Specialist',
    },
  ...
  let totalTests = policyTests.length;
  let passedTests = 0;

  console.log('Testing RBAC policy enforcement...\n');

  for (const test of policyTests) {
    try {
      const res = await api.request(test.method as HttpMethod, test.path);
      const allowed = res.status < 400;

      console.log(`[${test.expectRole}] ${test.method} ${test.path}`);
      console.log(`   Policy: ${test.description}`);
      console.log(`   Result: ${allowed ? '✅ Allowed' : '❌ Denied'} (${res.status})`);

      if (allowed) {
        console.log('   ✅ Policy enforced correctly');
        passedTests++;
      } else {
        console.log('   ❌ Policy not enforced - access should be allowed');
      }
    } catch (error) {
      console.log(`[${test.expectRole}] ${test.method} ${test.path}`);
      console.log(`   Policy: ${test.description}`);
  ...
  console.log('📊 RBAC Policy Validation Results:');
  console.log(`   Total policies: ${totalTests}`);
  console.log(`   Passed: ${passedTests} ✅`);
  console.log(`   Failed: ${totalTests - passedTests} ❌`);
  console.log(`   Success rate: ${((passedTests / totalTests) * 100).toFixed(1)}%`);
...
async function handleRBACMultiTenantTest(api: ApiClient) {
  console.log('🏢 PosalPro MVP2 - Multi-Tenant RBAC Test\n');

  const tenants = [
    { id: 'tenant-1', name: 'Tenant Alpha', expectedAccess: true },
    { id: 'tenant-2', name: 'Tenant Beta', expectedAccess: false },
    { id: 'tenant-default', name: 'Default Tenant', expectedAccess: true },
  ];

  const tenantEndpoints = [
    { method: 'GET', path: '/api/tenant/entitlements', description: 'Tenant entitlements access' },
    { method: 'GET', path: '/api/proposals', description: 'Tenant proposal access' },
    { method: 'GET', path: '/api/customers', description: 'Tenant customer access' },
  ];
  ...
  for (const tenant of tenants) {
    ...
    totalTests += tenantEndpoints.length;

    for (const endpoint of tenantEndpoints) {
      try {
        // Test with tenant-specific URL
        const tenantUrl = `${(api as any).baseUrl}?tenantId=${tenant.id}`;
        const res = await fetch(tenantUrl + endpoint.path, {
          method: endpoint.method,
          headers: (api as any).getHeaders(),
        });

        const allowed = res.status < 400;
        const expected = tenant.expectedAccess;
        const result = allowed === expected ? '✅' : '❌';

        console.log(
          `[${tenant.name}] ${endpoint.method} ${endpoint.path} → ${allowed ? 'Allowed' : 'Denied'} (${res.status}) | Expected: ${expected ? 'Allowed' : 'Denied'} ${result}`
        );
        ...
        console.log(
          `[${tenant.name}] ${endpoint.method} ${endpoint.path} → ❌ Error: ${error instanceof Error ? error.message : 'Unknown error'}`
        );
      }
    }
  }

  console.log(`\n📊 Multi-Tenant RBAC Test Results:`);
  console.log(`   Total tests: ${totalTests}`);
  console.log(`   Passed: ${passedTests} ✅`);
  console.log(`   Failed: ${totalTests - passedTests} ❌`);
  console.log(`   Success rate: ${((passedTests / totalTests) * 100).toFixed(1)}%`);
...
async function handleRBACPerformanceTest(api: ApiClient) {
  console.log('⚡ PosalPro MVP2 - RBAC Performance Test\n');

  const performanceTests = [
    { method: 'GET', path: '/api/admin/roles', iterations: 50, description: 'Admin roles listing' },
    { method: 'GET', path: '/api/proposals', iterations: 100, description: 'Proposal listing' },
    { method: 'GET', path: '/api/customers', iterations: 75, description: 'Customer listing' },
    { method: 'GET', path: '/api/products', iterations: 60, description: 'Product listing' },
    {
      method: 'GET',
      path: '/api/tenant/entitlements',
      iterations: 25,
  ...
  for (const test of performanceTests) {
    ...
    for (let i = 0; i < test.iterations; i++) {
      ...
      try {
        ...
        if (res.status >= 400) {
          console.log(`  Iteration ${i + 1}: ❌ ${res.status} (${end - start}ms)`);
        } else {
          console.log(`  Iteration ${i + 1}: ✅ ${res.status} (${end - start}ms)`);
        }
      } catch (error) {
        const end = Date.now();
        times.push(end - start);
        console.log(`  Iteration ${i + 1}: ❌ Error (${end - start}ms)`);
      }
    }
    ...
    console.log(`\n  📊 Performance Results for ${test.description}:`);
  ...
  console.log('\n📊 Overall RBAC Performance Summary:');
  console.log('────────────────────────────────────────');
  console.log('Test                    | Avg (ms) | P95 (ms)');
  console.log('────────────────────────────────────────');

  for (const result of results) {
    console.log(
      `${result.test.padEnd(23)} | ${result.avg.toFixed(2).padStart(8)} | ${result.p95.toFixed(2).padStart(8)}`
    );
  }
  ...
  console.log('────────────────────────────────────────');
  console.log(
    `Overall Average         | ${overallAvg.toFixed(2).padStart(8)} | ${overallP95.toFixed(2).padStart(8)}`
  );
}
...
/**
 * Check data consistency between list and detail views for a specific entity
 */
async function handleDataConsistencyCheck(api: ApiClient, entity: string, id?: string) {
  console.log(`🔍 PosalPro MVP2 - Data Consistency Check (${entity})\n`);

  try {
    const issues = [];

    if (entity === 'proposals') {
      await checkProposalConsistency(api, id);
    } else if (entity === 'customers') {
      await checkCustomerConsistency(api, id);
    } else if (entity === 'products') {
      await checkProductConsistency(api, id);
    } else {
    ...
    if (issues.length === 0) {
      ...
      console.log('List and detail views are consistent for this entity.');
    } else {
...
/**
 * Check consistency for all entities
 */
async function handleDataConsistencyCheckAll(api: ApiClient) {
  console.log('🔍 PosalPro MVP2 - Comprehensive Data Consistency Check\n');

  const entities = ['proposals', 'customers', 'products'];
  const results = [];

  for (const entity of entities) {
    console.log(`\n📋 Checking ${entity.toUpperCase()} consistency...`);
    try {
      if (entity === 'proposals') {
        await checkProposalConsistency(api);
      } else if (entity === 'customers') {
        await checkCustomerConsistency(api);
      } else if (entity === 'products') {
        await checkProductConsistency(api);
      }
      results.push(`${entity}: ✅ PASSED`);
    } catch (error) {
      results.push(
        `${entity}: ❌ FAILED - ${error instanceof Error ? error.message : 'Unknown error'}`
      );
    }
  }
  ...
  if (failed === 0) {
    console.log('\n✅ ALL ENTITIES PASSED DATA CONSISTENCY CHECKS!');
  } else {
    console.log(`\n🚨 FOUND ${failed} ENTITY CONSISTENCY ISSUES!`);
  }
}
 ...
 */
async function handleDataConsistencyValidate(api: ApiClient, entity: string, id: string) {
  console.log(`🔍 PosalPro MVP2 - Data Validation (${entity}: ${id})\n`);

  try {
    if (entity === 'proposals') {
      await validateProposalData(api, id);
    } else if (entity === 'customers') {
      await validateCustomerData(api, id);
    } else if (entity === 'products') {
      await validateProductData(api, id);
    } else {
      console.log(`❌ Unknown entity: ${entity}`);
      return;
    }
...
// ====================================
// 🆕 ENTITY-SPECIFIC CONSISTENCY CHECKERS
// ====================================

/**
 * Check proposal data consistency between list and detail views
 */
async function checkProposalConsistency(api: ApiClient, specificId?: string) {
  console.log('  📝 Checking proposal data consistency...');

  try {
    let listResponse, detailResponse;

    if (specificId) {
      // Check specific proposal
      console.log(`    🔍 Checking proposal: ${specificId}`);

      // Get from list API
      const listResult = await api.request('GET', '/api/proposals?limit=10');
      const listProposal = listResult.data?.items?.find((p: any) => p.id === specificId);

      if (!listProposal) {
        console.log(`    ⚠️ Proposal ${specificId} not found in list view`);
        return;
      }

      // Get from detail API
      const detailResult = await api.request('GET', `/api/proposals/${specificId}`);
      const detailProposal = detailResult.data;

      // Compare key fields
      const inconsistencies = compareProposalData(listProposal, detailProposal);

      if (inconsistencies.length > 0) {
        console.log(`    🔴 FOUND INCONSISTENCIES:`);
        inconsistencies.forEach(issue => console.log(`      - ${issue}`));
      } else {
        console.log(`    ✅ Proposal ${specificId} is consistent between list and detail views`);
      }
    } else {
      // Check all proposals in list
      console.log('    🔍 Checking all proposals in list...');

      const listResult = await api.request('GET', '/api/proposals?limit=20');
      const proposals = listResult.data?.items || [];

      console.log(`    📊 Found ${proposals.length} proposals to check`);

      for (const proposal of proposals) {
        try {
          const detailResult = await api.request('GET', `/api/proposals/${proposal.id}`);
          const detailProposal = detailResult.data;

          const inconsistencies = compareProposalData(proposal, detailProposal);
          ...
          console.log(
            `    ⚠️ Failed to fetch detail for proposal ${proposal.id}: ${error instanceof Error ? error.message : 'Unknown error'}`
          );
        }
      }
    }
    ...
    console.error(
      '    ❌ Error checking proposal consistency:',
      error instanceof Error ? error.message : 'Unknown error'
    );
  }
}
...
/**
 * Compare proposal data between list and detail views
 */
function compareProposalData(listProposal: any, detailProposal: any): string[] {
  const issues = [];

  // Check basic fields
  if (listProposal.title !== detailProposal.title) {
    issues.push(`Title mismatch: "${listProposal.title}" vs "${detailProposal.title}"`);
  }

  if (listProposal.status !== detailProposal.status) {
    issues.push(`Status mismatch: "${listProposal.status}" vs "${detailProposal.status}"`);
  }

  // Check value calculations
  const listValue = calculateProposalValue(listProposal);
  const detailValue = calculateProposalValue(detailProposal);

  if (Math.abs(listValue - detailValue) > 0.01) {
    issues.push(`Value mismatch: $${listValue.toFixed(2)} vs $${detailValue.toFixed(2)}`);
  }

  // Check customer information
  if (listProposal.customer?.name !== detailProposal.customer?.name) {
    issues.push(
      `Customer name mismatch: "${listProposal.customer?.name}" vs "${detailProposal.customer?.name}"`
    );
  }
...
/**
 * Calculate proposal value from either stored value or products
 */
function calculateProposalValue(proposal: any): number {
  if (proposal.products && proposal.products.length > 0) {
    return proposal.products.reduce((sum: number, product: any) => {
      const productTotal =
        typeof product.total === 'string' ? parseFloat(product.total) || 0 : product.total || 0;
      return sum + productTotal;
    }, 0);
  }
  return typeof proposal.value === 'string' ? parseFloat(proposal.value) || 0 : proposal.value || 0;
}

/**
 * Check customer data consistency
 */
async function checkCustomerConsistency(api: ApiClient, specificId?: string) {
  console.log('  👥 Checking customer data consistency...');

  try {
    // For now, just log that customer consistency checking is available
    console.log('    ✅ Customer consistency checking is available (implementation in progress)');

    if (specificId) {
      console.log(`    🔍 Would check customer: ${specificId}`);
    } else {
      console.log('    📊 Would check all customers for consistency');
    }
  } catch (error) {
    console.error(
      '    ❌ Error checking customer consistency:',
      error instanceof Error ? error.message : 'Unknown error'
    );
  }
}
 ...
 */
async function checkProductConsistency(api: ApiClient, specificId?: string) {
  console.log('  📦 Checking product data consistency...');

  try {
    // For now, just log that product consistency checking is available
    console.log('    ✅ Product consistency checking is available (implementation in progress)');

    if (specificId) {
      console.log(`    🔍 Would check product: ${specificId}`);
    } else {
      console.log('    📊 Would check all products for consistency');
    }
  } catch (error) {
    console.error(
      '    ❌ Error checking product consistency:',
      error instanceof Error ? error.message : 'Unknown error'
    );
  }
}
 ...
 * Validate specific entity data
 */
async function validateProposalData(api: ApiClient, id: string) {
  console.log(`  🔍 Validating proposal data: ${id}`);

  try {
    const response = await api.request('GET', `/api/proposals/${id}`);

    if (response.data) {
      console.log('    ✅ Proposal data structure is valid');
      console.log(`    📊 Title: ${response.data.title}`);
      console.log(`    💰 Value: $${calculateProposalValue(response.data).toFixed(2)}`);
      console.log(`    👥 Customer: ${response.data.customer?.name}`);
      console.log(`    📦 Products: ${response.data.products?.length || 0} items`);
    }
  } catch (error) {
    console.error(
      `    ❌ Error validating proposal ${id}:`,
      error instanceof Error ? error.message : 'Unknown error'
    );
  }
}

async function validateCustomerData(api: ApiClient, id: string) {
  console.log(`  🔍 Validating customer data: ${id}`);

  try {
    const response = await api.request('GET', `/api/customers/${id}`);
    ...
    console.error(
      `    ❌ Error validating customer ${id}:`,
      error instanceof Error ? error.message : 'Unknown error'
    );
  }
}

async function validateProductData(api: ApiClient, id: string) {
  console.log(`  🔍 Validating product data: ${id}`);

  try {
    const response = await api.request('GET', `/api/products/${id}`);

    if (response.data) {
      console.log('    ✅ Product data structure is valid');
      console.log(`    📦 Name: ${response.data.name}`);
      console.log(`    🏷️ Category: ${response.data.category}`);
      console.log(`    💰 Price: $${response.data.price?.toFixed(2)}`);
      console.log(`    📊 Stock: ${response.data.stock || 'N/A'}`);
    }
  } catch (error) {
    console.error(
      `    ❌ Error validating product ${id}:`,
      error instanceof Error ? error.message : 'Unknown error'
    );
  }
}
