...
export function useCustomers(
  ...
  const queryKey = CUSTOMERS_QUERY_KEYS.list(params);
  console.log('useCustomers: Query key:', queryKey);
  ...
  return useQuery({
    ...
    queryFn: async (): Promise<CustomersResponse> => {
      ...
      try {
        console.log('useCustomers: Making API call to:', `customers?${searchParams.toString()}`);
        ...
        if (!response.ok) {
          console.error('useCustomers: API returned ok=false:', response);
          throw new Error('Failed to fetch customers');
        }

        // Transform the API response to match the expected CustomersResponse format
        const customersResponse: CustomersResponse = {
          customers: response.data.items,
          pagination: {
            page: params.page || 1,
            limit: params.limit || 20,
            total: response.data.items.length, // API doesn't return total count
        ...
        console.log('useCustomers: Transformed Response:', customersResponse);

        return customersResponse;
      } catch (error) {
        console.error('useCustomers: Error caught:', error);
