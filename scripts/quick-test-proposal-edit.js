...
/**
 * PosalPro MVP2 - Quick Proposal Edit Test
 * Quick test of proposal edit functionality without browser
 *
 * Usage:
 *   node scripts/quick-test-proposal-edit.js
 */
...
function testAuthentication() {
  ...
  const loginCommand = `login ${TEST_CONFIG.adminEmail} '${TEST_CONFIG.adminPassword}'`;
...
function testProposalUpdate(proposalId) {
  ...
  const updateData = {
    title: `Quick Test Update - ${new Date().toISOString()}`,
    description: 'Quick test update',
  };
...
function runQuickTest() {
  log(`${colors.cyan}🚀 PosalPro MVP2 - Quick Proposal Edit Test${colors.reset}`, 'cyan');
  ...
  try {
    ...
    log(`\n${colors.green}🎉 Quick test completed successfully!${colors.reset}`, 'green');
