...
/**
 * Tenant-Aware Database Synchronization
 *
 * This script synchronizes data from local to cloud database,
 * handling the multi-tenant schema differences.
 */
...
console.log('🔄 Tenant-Aware Database Synchronization');
console.log('=========================================\n');

class TenantAwareSync {
  constructor() {
  ...
  async ensureDefaultTenant() {
    try {
      ...
      console.error('❌ Error ensuring default tenant:', error.message);
      throw error;
    }
  }
      ...
      for (const customer of localCustomers.rows) {
        try {
          ...
          if (existingCustomer.rows.length === 0) {
            await this.cloudClient.query(`
              INSERT INTO customers (id, "tenantId", name, email, industry, "createdAt", "updatedAt")
      ...
      for (const product of localProducts.rows) {
        try {
          ...
          if (existingProduct.rows.length === 0) {
            await this.cloudClient.query(`
              INSERT INTO products (id, "tenantId", name, description, price, category, "createdAt", "updatedAt")
  ...
  async performTenantAwareSync() {
    try {
      console.log('🚀 Starting tenant-aware synchronization...');
      ...
      if (this.syncStats.errors === 0) {
        console.log('\n🎉 Tenant-aware sync completed successfully!');
        console.log(`🏢 All data synced to tenant: ${this.defaultTenantId}`);
      } else {
        console.log('\n⚠️  Tenant-aware sync completed with errors');
      }

    } catch (error) {
      console.error('❌ Tenant-aware sync failed:', error.message);
      throw error;
    }
  }
}
...
async function main() {
  const sync = new TenantAwareSync();

  try {
    await sync.connect();
    await sync.performTenantAwareSync();
  } catch (error) {
