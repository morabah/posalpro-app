...
class ReactErrorMonitor {
  ...
  analyzeLogMessage(message) {
    ...
    if (
      message.includes('Enhanced performance monitoring') &&
      ...
      if (this.recentPerformanceLogs.length > 20) {
        ...
        console.log(
          `🚨 DETECTED: Performance monitoring spam #${this.errorCounts.performanceSpam}`
        );
  ...
  calculateSeverity() {
    const { infiniteLoops, errorBoundaries, useEffectIssues, fastRefreshIssues, performanceSpam } =
      this.errorCounts;
  ...
  generateRecommendations() {
    ...
    if (this.errorCounts.infiniteLoops > 0) {
      recommendations.push(
        '🚨 CRITICAL: Fix React infinite loops in useAdvancedPerformanceOptimization.ts:501 and RealTimeAnalyticsOptimizer.tsx:434 - add proper dependency arrays to useEffect hooks'
      );
    }

    if (this.errorCounts.errorBoundaries > 0) {
      recommendations.push(
        '🚨 CRITICAL: React components are crashing - investigate RealTimeAnalyticsOptimizer component for state management issues'
      );
    }

    if (this.errorCounts.useEffectIssues > 0) {
      recommendations.push(
        '🚨 CRITICAL: Add dependency arrays to useEffect hooks to prevent infinite re-renders'
      );
    }

    if (this.errorCounts.fastRefreshIssues > 0) {
      recommendations.push(
        '⚠️ HIGH: Fix Fast Refresh issues - ensure all React components are properly exported and not mixed with non-React exports'
      );
    }

    if (this.errorCounts.performanceSpam > 0) {
      recommendations.push(
        '⚠️ HIGH: Reduce performance monitoring frequency - current implementation causes excessive logging overhead'
      );
    }

    if (recommendations.length === 0) {
      recommendations.push(
        '✅ No critical issues detected - application performance monitoring looks healthy'
      );
    }
